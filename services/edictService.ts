...
export interface EdictTopic {
  ...
  linkedGoals: EdictLinkedGoals; 
  collapsed?: boolean;
  studyLevelId?: string | null; // Novo Campo: ID do Nível de Estudo
  observation?: string; // Added observation field
}
...
export const addEdictTopic = async (planId: string, disciplineId: string, name: string) => {
  const structure = await getEdict(planId);
...
export const addEdictSubtopic = async (planId: string, disciplineId: string, topicId: string, name: string) => {
  const structure = await getEdict(planId);
  const discipline = structure.disciplines.find(d => d.id === disciplineId);
